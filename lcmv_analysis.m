...
% Written by Robert Seymour (ABC) - March 2016
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for i=1:length(noise_level)
    for j = 1:length(maxfilter_setting)
        disp(['Performing source localisation using the visual data from ' noise_level{i} ' with Maxfilter parameters ' maxfilter_setting{j}])
        
        %% Load the required variables you computed earlier
        load(sprintf('D:\\RS_MC_analysis\\data_clean\\%s_%s_visual_data_clean.mat',noise_level{i},maxfilter_setting{j}))
        ...
        cd('D:\RS_MC_analysis\visual_source')
        ...
        comp = ft_componentanalysis(cfg, data_visual);
        
        cfg = [];
        cfg.updatesens = 'yes';
        cfg.component = comp.label(numcomponent:end);
        data_visual = ft_rejectcomponent(cfg, comp);
        
        %% Downsample - necessary step if you want your analysis to run this year!
        cfg = [];
        cfg.resamplefs = 250;
        cfg.detrend = 'no';
        data_visual_200 = ft_resampledata(cfg,data_visual);
        ...
        data_filtered = ft_preprocessing(cfg,data_visual_200)
        ...
        save([noise_level{i} '_' maxfilter_setting{j} '_visual_sourcepre'],'sourcepreS1','-v7.3');
        save([noise_level{i} '_' maxfilter_setting{j} '_visual_sourcepost'],'sourcepstS1','-v7.3');
...
maxfilter_setting = {'quat','sss'}; % maxfilter parameters
...
for i=1:length(noise_level)
    for j = 1:length(maxfilter_setting)
        ...
        %% Load the required variables you computed earlier
        load(sprintf('D:\\RS_MC_analysis\\visual_source\\%s_%s_visual_sourcepost.mat',noise_level{i},maxfilter_setting{j}))
        load(sprintf('D:\\RS_MC_analysis\\visual_source\\%s_%s_visual_sourcepre.mat',noise_level{i},maxfilter_setting{j}))
        
        
        cd('D:\RS_MC_analysis\visual_source\stats')
        ...
        system(['D:\Software\workbench\bin_windows64\wb_command -volume-to-surface-mapping D:\RS_MC_analysis\visual_source\stats\' noise_level{i} '_' maxfilter_setting{j} '_stat_nifti.nii ' 'D:\Software\workbench\bin_windows64\Conte69_atlas-v2.LR.32k_fs_LR.wb\32k_ConteAtlas_v2\Conte69.L.midthickness.32k_fs_LR.surf.gii D:\RS_MC_analysis\visual_source\stats\gifti\' noise_level{i} '_' maxfilter_setting{j} '_LEFT.shape.gii -trilinear'])
        system(['D:\Software\workbench\bin_windows64\wb_command -volume-to-surface-mapping D:\RS_MC_analysis\visual_source\stats\' noise_level{i} '_' maxfilter_setting{j} '_stat_nifti.nii ' 'D:\Software\workbench\bin_windows64\Conte69_atlas-v2.LR.32k_fs_LR.wb\32k_ConteAtlas_v2\Conte69.R.midthickness.32k_fs_LR.surf.gii D:\RS_MC_analysis\visual_source\stats\gifti\' noise_level{i} '_' maxfilter_setting{j} '_RIGHT.shape.gii -trilinear'])
    end
end
...
for i=1:length(noise_level)
    for j = 1:length(maxfilter_setting)
        ...
        %% Load the required variables you computed earlier
        load(sprintf('D:\\RS_MC_analysis\\visual_source\\stats\\%s_%s_stat.mat',noise_level{i},maxfilter_setting{j}))
        
        stat_out(i,j) = max(stat.stat);
    end
end
